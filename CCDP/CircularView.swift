...
class CircularView: UIView {
    ...
    func start(){
        motionManager.startDeviceMotionUpdates(to: OperationQueue.main) { (motion, err) in
            ...
            self.gravity.setAngle(CGFloat(rotation), magnitude: 0.5)
        }
    ...
    private func createDrop()->Drop{
        let frame = CGRect(x: bounds.width/2-5, y: 0, width: 10, height: 10)
